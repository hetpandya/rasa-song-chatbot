        ...
        if (response.length < 1) {
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("custom")) {
                    ...
                    if (response[i].custom.payload == "audio") {
                        audio_url = response[i].custom.url;
                        console.log(audio_url);

                        var BotResponse = '<div class="botMsg"> <audio autoplay="true" type="audio/webm" controls="true"  src="' + audio_url + '"/></div>'
                        $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                    }
                }
            }
